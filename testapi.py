...
# db_user = os.getenv('SQL_SERVER_USER', 'nirbhay')
# db_password = os.getenv('SQL_SERVER_PASSWORD', 'admin@1234')
# db_host = os.getenv('SQL_SERVER_HOST', 'dq-sql-server.database.windows.net')  # Docker Compose service name
# db_port = os.getenv('SQL_SERVER_PORT', '1433')
# db_name = os.getenv('SQL_SERVER_DB', 'exception_database')

# # Configure the SQL Server database URI for SQLAlchemy ORM
# DATABASE_URI = (
...
# DATABASE_URI = 'mssql+pyodbc://nirbhay:admin@1234@dq-sql-database-server-1.database.windows.net/exception_database?driver=ODBC+Driver+18+for+SQL+Server'
# DATABASE_URI = 'mysql+pymysql://nirbhay:Shyam$2002@localhost:3306/exception_database'

from sqlalchemy.engine import URL
connection_string = "DRIVER={ODBC Driver 18 for SQL Server};SERVER=dq-sql-server.database.windows.net;DATABASE=exception_database;UID=nirbhay;PWD=admin@1234"
DATABASE_URI = URL.create("mssql+pyodbc", query={"odbc_connect": connection_string})
...
def data_profiling():
    try:
        table_name = 'realpolicyclaims'
        page_size = 10000  # Number of records to fetch per page
        page_number = int(request.args.get('page', 1))  # Page number from query parameter
        
        # Calculate the offset
        offset = (page_number - 1) * page_size
        
        # Load the data in chunks using SQL Server syntax for pagination
        query = f"""
        SELECT * FROM {table_name}
        ORDER BY (SELECT NULL)  -- Required for OFFSET to work, or use a column name
        OFFSET {offset} ROWS
        ...
        response = {
            'page': page_number,
            'page_size': page_size,
            'total_records': int(pd.read_sql_query(f"SELECT COUNT(*) FROM {table_name}", engine).iloc[0, 0]),
            'data': table_data
        }
